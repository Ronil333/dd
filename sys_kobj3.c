...
static int __init sysfsexample_module_init(void)
{
	...
	ret = sysfs_create_file(mykobj,&my_first.attr);
	ret = sysfs_create_file(mykobj,&my_second.attr);
        return ret;
}

static void __exit sysfsexample_module_exit(void)
{
	sysfs_remove_file(mykobj,&my_first.attr);
	sysfs_remove_file(mykobj,&my_second.attr);
	if(mykobj)
	{
		kobject_put(mykobj);
		mykobj = NULL;
